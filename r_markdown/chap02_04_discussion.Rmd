...
<br />
There are a lot of differnces in output per worker across countries. It seems that all values between 0 and 250000 are present in this graph. But how does this compare to the distribution of capital per worker?
...
<br />
The distribution of capital per worker is wider than the distirbution of output per worker. From eyeballing we would get that the corresponding factor is about 4, i.e. the distribution spans from 0 to 1000000. We can be a bit more precise by looking at the quantiles of the two distributions. 
...
We now do a growth accounting exercise for the US. We therefore have to load the US dataset again and calculate the relevant statistics, i.e. GDP per worker, output per worker as well as the labor share.
...
<br >
By an large, we find the Solow residual to me the more important driving factor of economic growth. Movements in the capital stock only add modestly to ecnomic perforamance. In a way, this is the same result we already discussed in the previous section using cross-country data.
