...
ramsey <- function(T0, T1, c0, k0, g_t, tau_k, g, alpha, delta, gamma, beta) {
  ...
  res <- data_frame(year=c(T0:T1), k, y, c, g_t, i, ir, s)
  return(res)
}
...
res <- solve_ramsey(k_star, gov_spend, 1, g, alpha, delta, gamma, beta)
...
transition <- ramsey(T0, T1, res[1], k_star, g_t, tau_k, g, alpha, delta, gamma, beta)
...
# solve transition path
res <- solve_ramsey(k_star, gov_spend, 2, g, alpha, delta, gamma, beta)
...
# solve transition path
res <- solve_ramsey(k_star, tax_rate, 3, g, alpha, delta, gamma, beta)
